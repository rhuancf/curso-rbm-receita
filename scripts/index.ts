...
async function getReceitas() {
  if(arrayAllRecipes.length === 0) {
    ...
    if (recipesInPage.length < 20) window.onscroll = null; 
...
function renderMultiple(receitas:Receita[] = arrayAllRecipes) {
  ...
  for (let i = 0; i < receitas.length; i++) {
      ...
      if (difficulty > 3) difficultyIcon = '<svg style="color:#f1f148" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 32 32"><path fill="currentColor" d="M30 30h-8V4h8zm-6-2h4V6h-4zm-4 2h-8V12h8zm-10 0H2V18h8z"/></svg>';
      if (difficulty > 5) difficultyIcon = '<svg style="color:#dd2424fc" xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" preserveAspectRatio="xMidYMid meet" viewBox="0 0 32 32"><path fill="currentColor" d="M30 30h-8V4h8zm-10 0h-8V12h8zm-10 0H2V18h8z"/></svg>';
...
async function renderSingle(recipeUrl:string) {
  ...
  if(selectedRecipe) {
    ...
    for (let i = 0; i < selectedRecipe.Ingredients.length; i++) {
      if (containerIngredients) containerIngredients.innerHTML += `<hr><p><label>${selectedRecipe.Ingredients[i]}</label></p>`
    }
    
    const containerDescription = document.getElementById("full-recipe-description");
    if(containerDescription) containerDescription.innerHTML += `<p><label>${selectedRecipe.Description}</label></p>`

    const containerMethod = document.getElementById("full-recipe-method");
    for (let i = 0; i < selectedRecipe.Method.length; i++) {
      if (containerMethod) containerMethod.innerHTML += `<p><label>${selectedRecipe.Method[i]}</label></p>`
    }
...
function filterByDifficulty(arrayRecipes:Receita[] = arrayAllRecipes) {
  const selectedDifficulty = (difficultyFilterSelect as HTMLSelectElement).value.split('/');
  ...
  if(minMax[0] > 0) {
    newArrayRecipes = newArrayRecipes.filter((recipe) => {
      if(recipe.Method.length > minMax[0] && recipe.Method.length <= minMax[1]) return true;
    });
...
function filterByIngredients(arrayRecipes:Receita[] = arrayAllRecipes) {
  const ingredients = (document.querySelector(".search-ingredients") as HTMLLabelElement).innerHTML.toLowerCase();
  ...
  arrayIngredients.forEach((ingredient)=>{
    newArrayRecipes = newArrayRecipes.filter((recipe) => {
      ...
      recipe.Ingredients.forEach((e) => {
        if(e.toLowerCase().includes(ingredient)) flag = true;
      })
      return flag;
    });
...
function filterByName(arrayRecipes:Receita[] = arrayAllRecipes) {
  const inputValue = (document.querySelector("#search-name-input") as HTMLInputElement).value.toLowerCase();
...
function addIngredient() {
  const ingredientsLabel = (document.querySelector(".search-ingredients") as HTMLLabelElement);
  const inputValue = (document.querySelector("#search-ingredients-input") as HTMLInputElement).value.toLowerCase();
  (document.querySelector("#search-ingredients-input") as HTMLInputElement).value = '';
  ingredientsLabel.innerText += inputValue + "/";

  (document.querySelector(".clear-ingredients-button") as HTMLLabelElement).classList.remove("hide");

}

function clearIngredients() {
  (document.querySelector(".search-ingredients") as HTMLLabelElement).innerHTML = '';
  (document.querySelector(".clear-ingredients-button") as HTMLLabelElement).classList.add("hide");
}

async function applyFilters() {
  arrayAllRecipes = [];
  let newArrayRecipes = await getReceitas();
  ...
  clearRender();
  if(arrayAllRecipes.length > 0) getReceitas();
...
(selectedDifficulty as HTMLInputElement).addEventListener("change", applyFilters);
}

function eventListenerHandleAddIngredientButtonClick() {
  const searchButton = document.querySelector(".add-ingredient-button");
(searchButton as HTMLInputElement).addEventListener("click", addIngredient);
}
...
(searchButton as HTMLInputElement).addEventListener("click", clearIngredients);
}

function eventListenerHandleSearchButtonClick() {
  const searchButton = document.querySelector(".search-button");
(searchButton as HTMLInputElement).addEventListener("click", applyFilters);
}

function eventListenerHandleHomeClick() {
  const homeButton = document.querySelector("#home-link");
(homeButton as HTMLInputElement).addEventListener("click", reload);
}
...
(homeButton as HTMLInputElement).addEventListener("click", renderAbout);
}

function eventListenerHandleBackButtonClick() {
  const backbutton = document.querySelector("#back-button");
(backbutton as HTMLDivElement).addEventListener("click", reload);
}
